...
filepath = {
    ...
    'MSG_JJAS' : [cnst.network_data +'/data/OBS/MSG_WA30/', [6,7,8,9], (2004,2018)],  # 2004, 2018
    ...
    'MSG_USER' : [cnst.network_data +'/data/OBS/MSG_WA30/', [7], (2006,2006)]
...
def run(dataset, CLOBBER=False):
    ...
    for yy in range((filepath[dataset])[2][0],((filepath[dataset])[2][1])+1):   # (2004,2016)
        ...
        for mm in (filepath[dataset])[1]:
            ...
            savefile = path + 'coresPower_'+tag.upper()+'_-40_15000km2_-50points_dominant_'+str(yy) + '_'+str(mm).zfill(2)+'.nc'#'blobMap_-40-700km2_-50-points_dominant_'+str(yy) + '_'+str(mm).zfill(2)+'.nc'
            ...
            gridll = np.load('/users/global/cornkle/shared/data/OBS/MSG_WA30/MSG_1640_580_lat_lon.npz')
            lon = gridll['lon']
            lat = gridll['lat']
...
def file_loop(passit):
    ...
    ds = ds.sel(lat=slice(3,23), lon=slice(-18,13))     #[-14, 2.5, 4, 11.5] cutout to remove dodgy boundaries
