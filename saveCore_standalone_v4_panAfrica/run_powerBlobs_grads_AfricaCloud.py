...
def _loop(passit):
    ...
    wObj.applyWavelet(normed='scale')                                           # Applies wavelet scale decomposition to image that was just read in.
    dummy, max_location = wObj.scaleWeighting(wtype='nflics3k')                 # Does the 3km dataset scale weighting similar to previous NFLICS to filter out "nicely looking" cores.
