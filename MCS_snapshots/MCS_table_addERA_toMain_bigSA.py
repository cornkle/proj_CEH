...
'SA_big' : [[19, 36, -36,-12], 'spac', 2, (9,12), (11,3), (11,3)],
}
...
def run_ERA5_regional(fi):
    ...
    try:
        era_pl = xr.open_dataset(
            inpath + 'pressure_levels/' + eregion + '/ERA5_' + str(edate.year) + '_' + str(edate.month).zfill(
                2) + '_' + str(edate.day).zfill(2) + '_' + eregion + '_pl.nc')
    except:
        print('ERA5 missing')
        return
    try:
        era_srfc = xr.open_dataset(
            inpath + 'surface/' + eregion + '/ERA5_' + str(edate.year) + '_' + str(edate.month).zfill(
                2) + '_' + str(edate.day).zfill(2) + '_' + eregion + '_srfc.nc')
    except:
    ...
    try:
        era_wi100 = xr.open_dataset(
            inpath + 'surface/'+eregion+'/100mWind_ERA5_' + str(edate.year) + '_' + str(edate.month).zfill(
                2) + '_'+ str(edate.day).zfill(2) + '_'+eregion+'_srfc.nc')
    except:
        print('ERA5 missing')
        return
    ...
    for ids in range(pr.size):
        ...
        if np.isfinite(tgrad):
            ...
            out['efgrad_acc'].append(np.nan)

            out['shgrad'].append(np.nan)
            out['lhgrad'].append(np.nan)  # era fluxes opposite sign convention
            out['smgrad'].append(np.nan)
