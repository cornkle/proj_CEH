...
def add_ERA_regional(lt_mcs_hours):
    ...
    for mreg in MREGIONS.keys():
        for yy in range(2000,2021):
            ...
            outfile = outfile.replace('.csv', '_ERA5.csv')
            ...
            for y, m, d in zip(msg['year'], msg['month'], msg['day']):
                udate = datetime.date(y, m, d)
                msg['lt_date'].append(glob_util.UTC_to_LT_date(udate, mreg))

            hour_mask = (msg['lt_hour']>= lt_hour_start) & (msg['lt_hour']<= lt_hour_end)
            ...
            mdic = ERA_dictionary()  # defaultdict(list)
