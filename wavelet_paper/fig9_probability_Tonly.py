...
def comp_t():
   ...
   # path = 'D://data/wavelet/saves/pandas/'
    ...
    psumm = np.concatenate(np.array(dic['circle_pc']))  # [(hour>15) & (hour<23)]
    tmin = np.concatenate(np.array(dic['circle_t']))

    psum2 = np.concatenate(np.array(dic2['circle_p'])[scales2<=35])
    psumm2 = np.concatenate(np.array(dic2['circle_pc'])[scales2 <= 35])
...
def plot():
   ...
   # fpath = 'D://data/wavelet/saves/pandas/'
    ...
    ax1.plot(center, hist,  linewidth=1.5 , marker='o', label='Temperature only', color='k', linestyle='-.')
    ax1.plot(center, hist2,  linewidth=1.5 , marker='o', color='k', label='Scales$\leq$35km', linestyle = ':')
    ax1.legend()
    ax1.minorticks_on()
   ...
   # ax11.minorticks_on()
    ...
    ax1.fill_between(center, low * 100, up * 100, alpha=0.3, color='r')
    ...
    ax22 = ax2.twinx()
    ax22.plot(cent, np.array(area)/1000, linestyle='', marker='o', color='grey', mec='k') #(prob2 - prob1) / prob1 * 100
    ax22.set_ylabel('Average MCS area ($10^{3}km^{2}$)')
    ax22.minorticks_on()
    ax2.fill_between(cent, low, up, alpha=0.3, color='r')
    ax2.fill_between(cent, low2, up2, alpha=0.3)
