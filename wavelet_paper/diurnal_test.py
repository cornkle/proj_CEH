...
def run():
    df = pkl.load(open('/users/global/cornkle/C_paper/wavelet/saves/pandas/old/3dmax_gt15000.pkl', 'rb'))

    # uscales = np.unique(df['scale'])
    # sum = np.array(df['circle_sum'])
    # nzz = np.array(df['circle_nz'])
    # val = np.array(df['circle_val'])
    # s30 = np.array(df['circle_g30'])
    # scales = np.array(df['scale'])
    uscales = np.unique(df['scale'])
    sum = np.array(df['sum'])
    nzz = np.array(df['sumnz'])
    val = np.array(df['sumvalid'])
    s30 = np.array(df['sum30'])
    scales = np.array(df['scale'])
  ...
  #   #scales = np.arange(15,202,20)
    scenter = uscales[0:-1] + (uscales[1::] - uscales[0:-1]) / 2
...
#    ############## PLOT ALL SYSTEMS, AVERAGE MAX SCALE Fig1
    ...
    for iind, s in enumerate(uscales):
        if iind == 0:
            continue
        sums = np.sum(sum[(scales>=uscales[iind-1]) & (scales<s) ])
        sums30 = np.sum(s30[(scales>=uscales[iind-1]) & (scales<s) ])
        nz = np.sum(nzz[(scales>=uscales[iind-1]) & (scales<s) ])
        valid = np.sum(val[(scales>=uscales[iind-1]) & (scales<s) ])
