...
from shared.utils import constants as cnst, u_statistics as us
...
def trend_all():
    ...
    for m in months:
        ...
        map.set_plot_params(levels=[-1.2,-1,-0.8,-0.6,-0.4,0.4, 0.6,0.8,1,1.2], cmap='RdBu', extend='both')  # levels=np.arange(-0.5,0.51,0.1), [-0.6,-0.4,-0.2,0.2,0.4,0.6]
        ...
        map.set_contour(shear_mean.values, interp='linear', colors='k', levels=np.arange(10,150,8)) #, levels=np.arange(50,300,8),

        map.set_plot_params(levels=np.array([-0.8,-0.6,-0.4,-0.2, -0.1, 0.1, 0.2,0.4, 0.6,0.8])*10, cmap='RdBu_r', extend='both')  # levels=np.arange(-0.5,0.51,0.1)
        dic = map.visualize(ax=ax3, title='Sensible heat flux trend and mean', cbar_title='W m-2 decade-1')
        contours = dic['contour'][0]
        plt.clabel(contours, inline=True, fontsize=7, fmt='%1.1f')
