...
#from matplotlib.pylab import roots
import yaml

REPO_ROOT = Path(__file__).resolve().parents[2]
CONFIG_DIR = REPO_ROOT / "configs" / "environments"
EXAMPLE_CONFIG = CONFIG_DIR / "paths.example.yml"
...
def load_paths(config_file: str | Path | None = None, allow_example: bool = False) -> PathConfig:
    ...
    raw_figures = config.get("figures", {}) or {}
    figures = _resolve_mapping(raw_figures, {**roots})

    return PathConfig(machine=machine, roots=roots, datasets=datasets, figures=figures, raw=config)
