...
def trend_all():
    ...
    mcs = '/media/ck/Elements/SouthAmerica/GPM/monthly/*.nc'
    ...
    da3 = da3['precipitation'].sel(lon=slice(box[0], box[1]), lat=slice(box[2],box[3]))
    ...
    for ids, m in enumerate(months):
        ...
        #ipdb.set_trace()
        tirtrend_out = xr.DataArray(tirtrend_unstacked, coords=[da3.lat, da3.lon], dims=['lat','lon'])
        tirtrend_out.name = 'tir'
        tirmean_out = xr.DataArray(tirm_mean, coords=[da3.lat, da3.lon], dims=['lat','lon'])
        ...
        if len(m) == 1:
            fp = fpath + 'GPM_only_trendmap_Allmonths_SIG_inPerc_since2000_'+str(m[0]).zfill(2)+'.png'
        else:
            fp = fpath + 'GPM_only_trendmap_' + str(m[0]).zfill(2) +'-'+ str(m[1]).zfill(2) + '.png'
        ...
        dic = map.visualize(ax=ax1, title=str(m)+': GPM Monthly precipitation change', cbar_title='% decade-1',addcbar=True)
