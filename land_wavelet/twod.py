...
"""
authors, last updates: D Belusic 2016, C Klein 2023. With input from indicated references.
Including pycwt python package.
...
AUTHOR
    Sebastian Krieger (pycwt), adapted by D Belusic & C Klein
...
def cwt2d(f, dxy, a, wavelet=Mexican_hat()):
    """
    ...
    RETURNS
        ...
        real_scales: calculated scales in 'real world units' (e.g. km or deg)
    """
...
def icwt2d(W, dxy, a, wavelet=Mexican_hat(), individual=False):
    """
    Inverse bi-dimensional continuous wavelet transform as in Wang and
    Lu (2010), equation [5].
    PARAMETERS
        W (array like):
            Wavelet transform (wavelet coefficients), the result of the cwt2d function.
        a (array like, optional):
            Wavelet scale parameter array.
        w (class, optional) :
            Mother wavelet class. Default is Mexican_hat()

        individual: keyword to toggle return of (False) full renconstruction of signal or (true) individual scale array.
    RETURNS
        iW (array like) :
            Inverse wavelet transform.
    ATTENTION: THE SCALE FACTORS ARE EMPIRICALLY BASED AND ONLY WORK WELL IF ALL RELEVANT SCALES ARE COVERED BY THE SCALE RANGE.
    IF IN DOUBT, TEST THE FULL SCALE RECONSTRUCTION VERSUS ORIGINAL SIGNAL!
    """
    dx = dxy
    dy = dxy
    ...
    for i, an in enumerate(a):
        psi_ft_bar = an * wavelet.psi_ft(an * k, an * l)
        W_ft = fft2(W[i, :, :], s=(L, K))
        iW[i, :, :] = ifft2(W_ft * psi_ft_bar, s=(L, K)) * wavelet.cdelta / an ** 2.    # "empirical" scale factor

    if individual:
        out_iW = iW[:, :l0, :k0].real # 3d array with all scales separate
    else:
        out_iW = iW[:, :l0, :k0].real.sum(axis=0)  # full reconstruction across all scales
