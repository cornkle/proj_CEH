...
def composite(rawhour):
    ...
    for y in np.arange(2003, 2019):
        ...
        m1 = MONTHS[0]
        m2 = MONTHS[1]
        outfile = path + "/"+REGION+"_LSTA_"+SENSOR+"_SWA_2012-2019_MCSTRACK_BOX-ANNUAL_PF_DAY_"+str(m1).zfill(2)+'-'+str(m2).zfill(2)+'_'+str(y)+'_h'+str(rawhour).zfill(2)+extag+".p"
...
def file_loop(fi):
    ...
    idss = 0
    for lat, lon in zip(fi['init_lat'], fi['init_lon']):
            ...
            try:
                kernel2, kernel3, cnt, init = cut_kernel(xpos, ypos, lsta_da, [init_lon,init_lat])
            except TypeError:
                continue
