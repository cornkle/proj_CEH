...
def perSys(REGION):
    ...
    tthresh = '-50'
    files = glob.glob(cnst.lmcs_drive + '/save_files/'+region+'initTime*.csv')
    ...
    for y in range(2012,2019):
        ...
        pkl.dump(mdic, open(cnst.lmcs_drive + 'ERA5_MCS_saveFiles/'+region+'_bulk_pf1_ERA0.25_hourly_'+str(y)+'.p',
                               'wb'))
