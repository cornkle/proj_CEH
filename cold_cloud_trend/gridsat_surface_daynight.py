...
file3 = '/users/global/cornkle/data/OBS/gridsat/gridsat_netcdf/circles/single/JJAS/*.nc'
...
if not os.path.isfile(path+'ds_present2004_'+mon+'_3UTC.nc'):
...
coord = [-8.55,-6.5,5.5,7,5.8,6.2, -7.6, -7.4]
name='Tai park'
#lake volta
...
srtm_on_ds = ds18_present.salem.lookup_transform(srtm)
...
map.set_plot_params(vmin=0., vmax=25, nlevels=9, cmap='viridis', extend='both')
...
map.set_plot_params(vmin=0., vmax=20, nlevels=9, cmap='viridis')
map.set_data(ds3_present.values)
map.visualize(ax=ax2, title=ds3_present.name)
...
map.set_plot_params(vmin=0., vmax=100, nlevels=5, cmap='viridis')
...
dsp = ds3_present.sel(lat=lats).mean(dim='lat')
...
plt.plot(ds18_present.lon, (dsp-dsp.min())/(dsp.max()-dsp.min()), color='darkblue', label=dsp.name, marker='o')
ax.plot(ds18_present.lon, (dsp2-dsp2.min())/(dsp2.max()-dsp2.min()), color='orangered', label=dsp2.name, marker='o')
...
ax.plot(ds18_present.lon,veg/100., color='green', label='Evergreen trees')
ax.plot(ds18_present.lon,  temp, color='grey', label='Deforestation')
