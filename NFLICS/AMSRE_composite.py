...
def composite(h):
    ...
    msg = (msg['small_scale'])[(msg['time.hour'] == h)  & (msg['time.minute'] == 0) & (
        msg['time.year'] >= 2006) & (msg['time.year'] <= 2010) & (msg['time.month'] >= 6)]

    msg = msg.sel(lat=slice(8,16), lon=slice(-17,-10)).load()
...
def file_loop(fi):
    ...
    lsta = lsta.sel(lon=slice(-14, 30), lat=slice(1, 23))
