...
def significance(signal, dt, scales, sigma_test=0, alpha=None,
    ...
    if sigma_test == 0:  # No smoothing, dof=dofmin, TC98 sec. 4
        ...
        if len(dof) != 2:
            print('DOF must be set to [s1, s2], '
                              'the range of scale-averages')
        if Cdelta == -1:
            print('Cdelta and dj0 not defined for %s with f0=%f' %
        ...
        navg = sel.size
        if navg == 0:
            print('No valid scales between %d and %d.') % (s1, s2)
        ...
        print('sigma_test must be either 0, 1, or 2.')
