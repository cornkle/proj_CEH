...
the percent loss data. Another NetCDF file can also be created containing
the tree canopy cover in 2000, aggregated to the 0.05 degree scale.
...
def aggregate_tile(loss_filename):
    ...
    # Find which 30m pixels experience net loss/net gain of forest from 2000-2019 
    ...
    loss_post_2012 = loss_data > 12
    # Net loss if forest lost after 2012, or lost before 2012 and no gain from 2000-2012
    overall_loss_bool = np.logical_or(loss_post_2012, np.logical_and(loss_pre_2012, gain_data == 0))
    ...
    for i in range(number_blocks):
        for j in range(number_blocks):
            ...
            if total_land_area > 0.:
                # Create percentage tree cover map for 2019
                ...
                # Compute change in percentage cover 2000-2019.
...
def write_aggregated_tile(loss_tile_filename):
    ...
    save_directory = '../data/hansen_0pt05'
    if not os.path.isdir(save_directory):
        os.makedirs(save_directory)
    save_filename_cover2000 = f'{save_directory}/Hansen_GFC_percent_cover_2000_{loss_tile_filename[-12:-4]}.nc'
    save_filename_loss = f'{save_directory}/Hansen_GFC_percent_net_loss_{loss_tile_filename[-12:-4]}.nc'
    if np.logical_and(os.path.exists(save_filename_cover2000), os.path.exists(save_filename_loss)):
        ...
        # Save aggregated data for percent forest loss 2000-2019
        loss_file = Dataset(save_filename_loss, 'w', format='NETCDF4')
        loss_file.history = "Created " + today.strftime("%d/%m/%y")
        loss_file.description = f'Percentage loss of global forest coverage from 2000-2019 on 0.05 deg CMG.'
