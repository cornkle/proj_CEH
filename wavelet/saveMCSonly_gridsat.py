...
def run():
    ...
    savefile = '/users/global/cornkle/MCSfiles/blob_map_35km_-78_0-3UTC.nc'
    ...
    das.to_netcdf('/users/global/cornkle/MCSfiles/blob_map_35km_-78_sum_0-3UTC.nc')
...
def file_loop(passit):
    ...
    if (np.int(strr[8:10]) > 3): #& (np.int(strr[8:10]) < 18): #(np.int(strr[4:6]) != 6) & #(np.int(strr[8:10]) != 3)
        print('Skip')
        return
    ...
    for min in min_list:
        ...
        for nb in scale_ind[::-1]:
           ...
           # maxout = maxoutt[nb, :, :]
            ...
            try:
                yy, xx = np.where((maxout == 1) & (outt <= -78) & ((wl >= np.percentile(wl[wl >= 0.5], 90)) & (wl > orig**.5) ))  # )& (wl > orig**.5) (wl >= np.percentile(wl[wl >= 0.1], 90)) )#(wl > orig**.5))#  & (wlperc > orig**.5))# & (wlperc > np.percentile(wlperc[wlperc>=0.1], 80)))# & (wlperc > np.percentile(wlperc[wlperc>=0.1], 80) ))  # & (wl100 > 5)
            except IndexError:
                continue
