...
MREGIONS = { 'Zambia' : [[21,33,-17,-8], 'tab', (), (), (1,2)]
 #    'WAf' : [[-18,25,4,25], 'spac', 0, (1,7), (8,12), (1,12)], # last is hourly offset to UCT # 12    # [-18,25,4,25]
...
def composite(rawhour):
    ...
    for y in np.arange(2002, 2019):
        for m in np.arange(1,3):
            for d in np.arange(1,32):



                msg = glob.glob('/prj/Africa_cloud/ch9_wavelet/'+str(y)+'/'+str(m).zfill(2)+'/'+str(d).zfill(2)+'/*1500.nc') # 17LT
                msg2 = glob.glob('/prj/Africa_cloud/ch9_wavelet/' + str(y) + '/' + str(m).zfill(2) + '/' + str(d).zfill(2) + '/*1300.nc') #15 LT
                ...
                for file in allinds:
                    ...
                    lons.append(f.max_lon.values)
                    lats.append(f.min_lon.values)

                dic[str(msg[0][0:8])]= (lons, lats)


        try:
            dic = u_parallelise.run_arrays(4, file_loop, dic, ['ano', 'regional', 'cnt', 'allcnt', 'init'])
        except:
        ...
        pkl.dump(dic, open(path + "/"+REGION+"_LSTA_"+SENSOR+"_TESTBED_2012-2019_BOX-FEB_PF_DAY_"+str(1).zfill(2)+'-'+str(2).zfill(2)+'_'+str(y)+'_h'+extag+".p", "wb"))
