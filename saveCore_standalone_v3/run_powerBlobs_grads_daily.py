...
def run(dataset, CLOBBER=False):
    ...
    for yy in range((filepath[dataset])[2][0],((filepath[dataset])[2][1])+1):
        ...
        path = cnst.network_data + '/MCSfiles/MSG_cores/' + str(yy) # (2004,2016)

        for mm in (filepath[dataset])[1]:

                tag = dataset[0:3].upper()

                 #'/prj/vera/cores_bigDomain/' #
                ...
                for dd in range(1, 32):
                    ...
                    savefile = path +'/coresPower_' + tag.upper() + '_-40_9-130km_-50points_dominant_' + str(
                        yy) + str(mm).zfill(2) + str(dd).zfill(
                        2) + '.nc'  # 'blobMap_-40-700km2_-50-points_dominant_'+str(yy) + '_'+str(mm).zfill(2)+'.nc'
                    ...
                    day_str = []
                    for f in files_str:
                        if str(yy)+str(mm).zfill(2)+str(dd).zfill(2) in f:
                            day_str.append(f)
