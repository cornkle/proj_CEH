...
def saveMCS_WA15():
    ...
    for _y in YRANGE:
        ...
        files = glob.glob(trmm_folder + '/'+str(_y) + '/*/*.nc4')# area=[-12, 12, 4, 9])   # [-15, 15, 4, 21], [-10, 10, 10, 20]

        for tf in files:


            t = xr.open_dataset(tf)
            da = t['HQprecipitation']
            ...
            for gi in goodinds:
                ...
                try:
                    dummyt = griddata(tpoints, tdic.values.flatten(), inter, method='linear')
                except ValueError:
                    continue
