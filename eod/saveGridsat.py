...
def saveYearly():
    ...
    years = np.arange(1983, 2014)  # list(next(os.walk(msg_folder))[1])

    for y in years:
        da = None
        if os.path.isfile(out + 'gridsat_WA_-50_' + str(y) + '.nc'):
            continue
        ...
        for f in files:
            ...
            labels, goodinds = ua.blob_define(df['tir'].values, -50, minmax_area=[83,25000], max_area=None) # 7.7x7.7km = 64km2 per pix in gridsat?
        ...
        da.to_netcdf(out + 'gridsat_WA_-50_' + str(y) + '.nc', encoding=enc)
        da.close()
