...
import pandas as pd
import os
from utils import u_parallelise, u_arrays as ua, constants as cnst, u_darrays
...
def composite(rawhour):
    ...
    for y in np.arange(2012, 2020):
        ...
        m1 = MONTHS[0]
        m2 = MONTHS[1]
        outfile = path + "/"+REGION+"_SM_"+SENSOR+"_SWA_2012-2019_MCSTRACK_BOX-ERA_DAY_"+str(m1).zfill(2)+'-'+\
        ...
        inmask =   ((msg['lt_init'] >= SENSOP+2) & (msg['tracktime'] >= TRACKTIME)  & (msg['ccs_area'] >= AREA) & \
                   ((msg['hour']==h)  | (msg['hour']==h1)  | (msg['hour']==h2)) & \
                    ...
                    ((np.abs(msg['londiff_loc-init'])>=INIT_DISTANCE)|(np.abs(msg['latdiff_loc-init'])>=INIT_DISTANCE)) & (msg['meanlat'] >=8) & (msg['meanlon'] <=12))
...
def file_loop(fi):
    ...
    str(daybefore.year) + str(daybefore.month).zfill(2) + str(daybefore.day).zfill(2)
    edate = str(daybefore.year) +'_' + str(daybefore.month).zfill(2) + '_' + str(daybefore.day).zfill(2)

    alt_path = cnst.lmcs_drive + 'ERA5/hourly/pressure_levels/WAf/'#'AMSR2/daily/25km/day_anom/'
    try:
        lsta = xr.open_dataset(alt_path + 'ERA5_' + edate + '_WAf_pl.nc')
    ...
    lsta = lsta.sel(time=(lsta['time.hour']==12), longitude=slice(box[0]-3, box[1]+3), latitude=slice(box[2]-3, box[3]+3)) # define SM region box to consider

    print('Doing ' + 'AMSR2_' + str(daybefore.year) + str(daybefore.month).zfill(2) + str(
        daybefore.day).zfill(2) + '.nc', date)


    lsta_da = lsta['u'].sel(level=650).squeeze()  # soil_moisture_c1
    ts_da = lsta['v'].sel(level=650).squeeze()
    #lsta_da = ts_da-lsta_da
...
############################
    ...
    for id in range(3): # 3 precip entries
        ...
        for lat, lon in zip(fi['pf_lat'+str(id+1)], fi['pf_lon'+str(id+1)]):
            ...
            try:
                point = lsta_da.sel(latitude=lat, longitude=lon, method='nearest', tolerance=0.2)
            except KeyError:
                #ipdb.set_trace()
            ...
            xpos = np.where(lsta_da['longitude'].values == plon)
            try:
                xpos = int(xpos[0])
            except:
                continue
            ypos = np.where(lsta_da['latitude'].values == plat)
            ypos = int(ypos[0])
            #ipdb.set_trace()
            try:
                kernel2, kernel3, cnt, init = cut_kernel(xpos, ypos, lsta_da, [init_lon,init_lat], wd=75)  #, wd=direct
            except TypeError:
                continue

            try:
                tskernel2, tskernel3, cntts, init = cut_kernel(xpos, ypos, ts_da, [init_lon,init_lat], wd=75)  #
            except TypeError:
                continue
...
def plot_sm_ts(rawhour):
        h = rawhour - (MREGIONS[REGION])[2]
        print('Hour: ', h)
        pin = cnst.network_data + 'data/GLOBAL_MCS/save_composites/' + "/" + REGION + "_SM_" + SENSOR + "_SWA_2012-2019_MCSTRACK_BOX-ERA_DAY_" + str(
        MONTHS[0]).zfill(2) + '-' + str(MONTHS[1]).zfill(2) + '_'
        y1 = 2013
        y2 = 2020

        dic = pkl.load(open(
            pin + str(y1) + '_h' + str(rawhour).zfill(2) +'_'+ extag+".p", "rb"))

        def coll(dic, h, year):
            print(h)
            core = pkl.load(open(
                pin + str(year) + '_h' + str(rawhour).zfill(2) +'_'+ extag+ ".p", "rb"))
        ...
        plt.quiver(np.arange(extent*2+1), np.arange(extent*2+1), ano, ano_t, scale=55, width=0.0015,
                   headwidth=7)
