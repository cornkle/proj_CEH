...
for ids, regs in enumerate(REGIONS):
        ...
        print('Hour: ', h)
        extag = '_day-1'
        pin = cnst.network_data + 'data/GLOBAL_MCS/save_composites/' + "/" + regs + "_LSTA_"+SENSOR+"_SWA_2012-2019_MCSTRACK_BOX-ANNUAL_PF_DAY_"+str(MONTHS[0]).zfill(2)+'-'+str(MONTHS[1]).zfill(2)+'_'
        y1 = 2003
        y2 = 2020
        ...
        plt.contourf(ano, cmap='rainbow', levels=np.arange(1,5), #np.linspace(thresh * -1, thresh, 10),
        ...
        rect = patches.Rectangle((30, 30), 60, 60, linewidth=0.5, edgecolor='k', facecolor='none')
        ax.add_patch(rect)
        ...
        plt.contourf(ano, cmap='RdBu_r', levels=np.linspace(thresh * -1, thresh, 20),
                     extend='both')
        plt.plot(extent, extent, 'bo')
        ax.set_xticks((np.linspace(0, 2 * extent, 9)))
        ax.set_xticklabels(((np.linspace(0, (2 * extent), 9) -extent)/20).round(1).astype(float))
        ...
        cb = plt.colorbar(label='K', format='%.2f')
        ...
        plt.colorbar(label='K', format='%.2f')
        plt.title(regs+' day0 ano-mean  | n='+str(np.max(dic['allcnt'])), fontsize=10)

plt.tight_layout()

plt.savefig(
    cnst.elements_drive + '/' + str(rawhour) + '/AllRegions_' + SENSOR + '_LSTA_' + str(MONTHS[0]).zfill(
        2) + '-' + str(
