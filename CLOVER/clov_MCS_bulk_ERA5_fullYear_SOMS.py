...
def perSys():
    ...
    #tthresh = '-50'
    files = glob.glob(cnst.network_data + 'MCSfiles/WA5000_5-10N_10W-10E_-50_afternoon_GPM_fullyear_SOMS/*.nc')
        ...
        for v in res:
            for k in keys:
                try:
                    ...
                    continue
        ooutf = cnst.network_data + 'data/CLOVER/saves/bulk_'+TTHRESH+'_5000km2_GPM_5-10N_10W10E_p15_ERA0.7_fullYear_hourly_SOMS_'+str(y)+'.p'      
        if os.path.isfile(ooutf):
           print('File exists, continue')
           return
        pkl.dump(mdic, open(ooutf, 'wb'))
...
def file_loop(f):
    ...
    y = dic['time.year'].item() 
    fout = cnst.network_data + 'data/CLOVER/saves/bulk_'+TTHRESH+'_5000km2_GPM_5-10N_10W10E_p15_ERA0.7_fullYear_hourly_SOMS_'+str(y)+'.p'
