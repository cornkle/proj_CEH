...
import pdb
import itertools
from utils import u_plot as uplot, constants_lappi as cnst
...
pos = np.where( (pp >= 1) & ((month==10)| (month<=4)))# np.where((pp >= 3) & (sh >= 8) &  (sh <= 30) &  (area<=700000) & ((month<=5) | (month>=10)) & (lat>=5) )   # 5 + 10 look nicest
...
shearbins = np.percentile(sh[(sh>=5) & (sh<=19)], np.linspace(0,100,nbs))
...
ax5.set_title('')
cbar = fig.colorbar(mappable, ticks=np.linspace(20,40,11)) # ticks=np.linspace(30,45,11)
