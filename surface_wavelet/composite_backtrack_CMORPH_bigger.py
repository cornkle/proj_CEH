...
def composite(h):
    ...
    pkl.dump(dic, open("/users/global/cornkle/figs/LSTA-bullshit/corrected_LSTA/system_scale/composite_backtrack_CMORPH_big"+str(hour).zfill(2)+".p", "wb"))
...
def file_loop(fi):
    ...
    pos = np.where( fi.values == 9 )  #(fi.values >= 5) & (fi.values < 65)
...
def plot_gewex(h):
    hour=h
    dic = pkl.load(open("/users/global/cornkle/figs/LSTA-bullshit/corrected_LSTA/system_scale/composite_backtrack_CMORPH_big"+str(hour).zfill(2)+".p", "rb"))
    ...
    contours = plt.contour((dic['prob']/ dic['cntp'])*100, extend='both', levels=np.arange(10,70,5)) # #, levels=np.arange(1,5, 0.5)
    plt.clabel(contours, inline=True, fontsize=9, fmt='%1.0f')

    ax.set_xticklabels(np.array((np.linspace(0, extent*2, 7) - extent) * 3, dtype=int))
    ax.set_yticklabels(np.array((np.linspace(0, extent*2, 7) - extent) * 3, dtype=int))
    ax.set_xlabel('km')
    ax.set_ylabel('km')
    ...
    plt.tight_layout()
    plt.savefig('/users/global/cornkle/figs/LSTA-bullshit/corrected_LSTA/system_scale/'+str(hour).zfill(2)+'_single.png')#str(hour).zfill(2)+'00UTC_lsta_fulldomain_dominant<60.png')
    plt.close()
