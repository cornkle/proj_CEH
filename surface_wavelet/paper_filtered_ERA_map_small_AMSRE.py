...
def composite(h, eh):
    ...
    msgin = msg[msg['SMwet']==2]#[(msg['initTime']>=12.5) & (msg['initTime']<h)]#[msg['SMwet']==2]
    ...
    for year in np.arange(2006, 2011):
        ...
        pkl.dump(dic, open(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_propagation_WET_noERAfilter_AMSRE"+str(eh) + "UTCERA"+str(hour).zfill(2)+'_'+str(year)+".p", "wb"))
        del dic
        print('Dumped file')
...
def file_loop(df):
    ...
    #topo = topo.sel(lat=slice(7,22.5), lon=slice(-13,13))
    ttopo = topo['h']
...
def plot_doug_all(h, eh):
    ...
    name = "ERA5_cores_propagation_noERAfilter_AMSRE"#"ERA5_composite_cores_AMSRE_w1_15k_minusMean"
...
def plot_doug_small(h, eh):
    ...
    plt.show()
    plt.savefig(
        cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/plots/forChris" + name + str(h).zfill(2) + '_' + str(eh).zfill(
            2) + '_AMSR_small.png')


def plot_doug_tiny(h, eh):

    dic = {}
    name = "ERA5_cores_initiation_noERAfilter_AMSRE"  # "ERA5_composite_cores_AMSRE_w1_15k_minusMean"
    ...
    contours = plt.contour(((dic['div'])/ dic['cnte'])*100, extend='both',  cmap='PuOr', levels=[-0.8,-0.6,-0.4,-0.2, 0.2,0.4,0.6,0.8]) # #, levels=np.arange(1,5, 0.5)
    qu = ax.quiver(xquiv, yquiv, u, v, scale=15)
    qk = plt.quiverkey(qu, 0.9, 0.02,1, '1 m s$^{-1}$',
                       labelpos='E', coordinates='figure')
    ...
    ax1 = f.add_subplot(224)
    #   plt.contourf(((dic['lsta'])/ dic['cnt']), extend='both',  cmap='RdBu_r', vmin=-1.5, vmax=1.5) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.contourf(((dic['theta_e']) / dic['cnte']), extend='both', cmap='RdBu', levels=np.linspace(-2.5,2.5,16)) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.colorbar(label=r'K')
    plt.plot(extent, extent, 'bo')
    #contours = plt.contour((dic['probmsg'] / dic['cntm']), extend='both', cmap='PuOr_r', levels=[-0.4,-0.3,-0.2,-0.1, 0,0.1,0.2,0.3,0.4]) #np.arange(-15,-10,0.5)
    contours = plt.contour((dic['skt'] / dic['cnte']), extend='both', cmap='PuOr_r',
                           levels=np.linspace(-2,2,15))  # np.arange(-15,-10,0.5)
    plt.clabel(contours, inline=True, fontsize=9, fmt='%1.2f')
    qu = ax.quiver(xquiv, yquiv, u, v, scale=15)
    qk = plt.quiverkey(qu, 0.9, 0.02,1, '1 m s$^{-1}$',
                       labelpos='E', coordinates='figure')
