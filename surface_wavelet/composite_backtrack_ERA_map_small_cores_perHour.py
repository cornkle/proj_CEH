...
def composite(h, eh):
    ...
    msgo = msg[((msg['time.hour'] >= 17) & (msg['time.hour'] <= 19) ) & (msg['time.minute'] == 0) & (
        msg['time.year'] <=2008) & (msg['time.month'] >=7) ]
    ...
    res = pool.map(file_loop, msgo[400:700])
    pool.close()
    ...
    pkl.dump(dic, open(cnst.network_data + "figs/LSTA/corrected_LSTA/new/composite_backtrack"+str(eh) + "UTCERA"+str(hour).zfill(2)+'_'+str(year)+"_small_cores.p", "wb"))
    del dic
    print('Dumped file')
...
def plot_doug(h, eh):
    ...
    dic = pkl.load(open(
        cnst.network_data + "figs/LSTA/corrected_LSTA/new/composite_backtrack"+str(eh) + "UTCERA"+str(h).zfill(2)+"_all_small_cores.p", "rb"))

    dic2 = pkl.load(open(
        cnst.network_data + "figs/LSTA/corrected_LSTA/new/composite_backtrack_CMORPH_"+ str(h).zfill(2) +"_cores.p", "rb")) #" + str(h).zfill(2) + "
 ...
 #   plt.contourf(((dic['lsta'])/ dic['cnt']), extend='both',  cmap='RdBu_r', vmin=-1.5, vmax=1.5) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/"+str(h).zfill(2)+'_cores.png')#str(hour).zfill(2)+'00UTC_lsta_fulldomain_dominant<60.png')
    #plt.close()
