...
def composite(h, eh):
    ...
    key = '2hOverlap'
    msgopen = pd.read_csv(cnst.network_data + 'figs/LSTA/corrected_LSTA/new/ERA5/core_txt/cores_gt15000km2_table_AMSRE_LSTA_tracking_2hOverlap_'+str(h)+'.csv', na_values=-999)
    hour = h
    ...
    for year in np.arange(2006, 2011):
        ...
        pkl.dump(dic, open(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_"+key+"_AMSRE_SMWET_AMSL_mini"+str(eh) + "UTCERA"+str(hour).zfill(2)+'_'+str(year)+".p", "wb"))
        del dic
        print('Dumped file')
...
def cut_kernel(xpos, ypos, arrlist, dist, era=False):
    ...
    return smoutlist,  scntlist, vdic,cntera
...
def get_previous_hours(storm_date, lsta_date, ehour, refhour):
    ...
    shear =  (cmm['u'].sel(level=650).squeeze() - cmm['u'].sel(level=level_low).squeeze() )
    shear_clim = (pl_clim['u'].sel(level=650).squeeze() - pl_clim['u'].sel(level=level_low).squeeze() ) #
    ...
    uwind_up = cmm['u'].sel(level=650).squeeze()
    vwind_up = cmm['v'].sel(level=650).squeeze()
    ...
    rva = cmm['pv'].sel(level=700).squeeze() - pl_clim['pv'].sel(level=700).squeeze()

    q = cmm['q'].sel(level=level_low).squeeze()
    qclim = pl_clim['q'].sel(level=level_low).squeeze()

    cm['shear'] = shear
    cm['shear_clim'] = shear_clim
    cm['u925'] = uwind_srfc
    cm['v925'] = vwind_srfc
...
def file_loop(df):
    ...
    str(prev_lsta_date.year) + str(prev_lsta_date.month).zfill(2) + str(prev_lsta_date.day).zfill(2)
    ...
    for dids, dit in df.iterrows():
        ...
        try:
            smoutlist, scntlist, vdic, cntera = cut_kernel(xpos, ypos, smlist, dist, era=probs_on_lsta)
        except TypeError:
            print('TypeError')
            continue
    ...
    outlist = smkernels + smcnt + [cnte_sum]
    outnames = ['lsta0', 'lsta-1', 'cnt0', 'cnt-1', 'cnte']
    for ek in edic.keys():
        outnames.append(ek)
...
def plot_doug_all(h, eh):
    ...
    name = "ERA5_cores_2hOverlap_AMSRE_SMWET_AMSL_mini"
    ...
    plt.contourf((dic['lsta-1'] / dic['cnt-1']), cmap='RdBu_r', levels=np.linspace(-2,2,16), extend='both') #-(rkernel2_sum / rcnt_sum)
 ...
 #   plt.contourf(((dic['lsta'])/ dic['cnt']), extend='both',  cmap='RdBu_r', vmin=-1.5, vmax=1.5) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.contourf(((dic['pv'])/ dic['cnte'])*10000000, extend='both',  cmap='PuOr', levels=np.linspace(-0.7,0.7,10)) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.colorbar(label='10$^{-2}$ s$^{-1}$', format='%1.3f')
    plt.plot(extent, extent, 'bo')
    contours = plt.contour((dic['t']-dic['tclim']) / dic['cnte'], extend='both', cmap='RdBu_r') #np.arange(-15,-10,0.5)
    plt.clabel(contours, inline=True, fontsize=9, fmt='%1.2f')
    ...
    plt.contourf(((dic['div']) / dic['cnte']) * 100, extend='both', cmap='PuOr', levels=np.linspace(-0.7, 0.7, 10))
...
def plot_doug_small(h, eh):
    ...
    #name = "ERA5_cores_2hOverlap_AMSRE_ALL_SM0FINITE"
    #name = "ERA5_cores_2hOverlap_AMSRE_WET_SMGT2-05"
    name = "ERA5_cores_2hOverlap_AMSRE_SMDRY_AMSL_mini"
...
def plot_timeseries_diff_coarse():
    ...
    outdic2 = {}
    dummyfile = glob.glob(
        cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_2hOverlap_AMSRE_SMWET_AMSL_mini*.p")
   ...
   # contours = plt.contour(ranges, yax,(outdic['v650_orig']/ outdic['cnte']), extend='both',colors='b', linewidths=0.5, levels=[-50,0,50])
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/plots/ERA5_amsre_"+str(h).zfill(2)+'_timeseries_AMSL_DIFF_coarse_mini.png')#str(hour).zfill(2)+'00UTC_lsta_fulldomain_dominant<60.png)
    plt.close()
...
def plot_timeseries_coarse_noStreamline():
    ...
    dummyfile = glob.glob(
        cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_2hOverlap_AMSRE_SMDRY_AMSL*.p")
    dummy = pkl.load(open(dummyfile[0], "rb"))
    file = "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_2hOverlap_AMSRE_SMDRY_AMSL_mini"
    ...
    levels = list(np.arange(-0.7, 0, 0.1)) + list(np.arange(0.1, 0.75, 0.1))
    ...
    #                        levels=np.arange(-3.5,0,0.5), linewidths=1)
    plt.clabel(contours, inline=True, fontsize=9, fmt='%1.2f')
   ...
   # contours = plt.contour(ranges, yax,(outdic['v650_orig']/ outdic['cnte']), extend='both',colors='b', linewidths=0.5, levels=[-50,0,50])
    ...
    plt.text(0.02, 0.03, 'v-wind anomaly 0-line', color='red', fontsize=14, transform=ax.transAxes)
    ...
    #contours = plt.contour(ranges, yax, diff['theta_e']-diff['theta_e_clim'],colors='white', levels=np.arange(1,3,0.5), linewidths=1.5)
    contours = plt.contour(ranges, yax, (diff['theta_e'] - diff['theta_e_clim'])-2*(diff['theta'] - diff['theta_clim'])-0.5, colors='white',
                           levels=np.arange(-3, 3, 0.5), linewidths=1.5)
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/plots/ERA5_amsre_"+str(h).zfill(2)+'_timeseries_SM_AMSL_'+tag+'_coarsemini.png')#str(hour).zfill(2)+'00UTC_lsta_fulldomain_dominant<60.png)
    plt.close()


def plot_timeseries_coarse_long():
    ...
    dummyfile = glob.glob(
        cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_2hOverlap_AMSRE_SMDRY_AMSL*.p")
    dummy = pkl.load(open(dummyfile[0], "rb"))
    file = "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_2hOverlap_AMSRE_SMWET_AMSL_mini"
   ...
   # contours = plt.contour(ranges, yax,(outdic['v650_orig']/ outdic['cnte']), extend='both',colors='b', linewidths=0.5, levels=[-50,0,50])
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/plots/ERA5_amsre_"+str(h).zfill(2)+'_timeseries_SM_AMSL_'+tag+'_coarselong.png')#str(hour).zfill(2)+'00UTC_lsta_fulldomain_dominant<60.png)
    plt.close()


def plot_timeseries_diff_coarse_long():
   ...
   # contours = plt.contour(ranges, yax,(outdic['v650_orig']/ outdic['cnte']), extend='both',colors='b', linewidths=0.5, levels=[-50,0,50])
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/plots/ERA5_amsre_"+str(h).zfill(2)+'_timeseries_AMSL_DIFF_coarse_mini_long.png')#str(hour).zfill(2)+'00UTC_lsta_fulldomain_dominant<60.png)
    plt.close()
...
def plot_doug_timeseries():
    ...
    dummyfile = glob.glob(
        cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_2hOverlap_AMSRE_SMDRY_AMSL_mini*.p")
    dummy = pkl.load(open(dummyfile[0], "rb"))
    file = "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_2hOverlap_AMSRE_SMDRY_AMSL_mini"
    ...
    ax.streamplot(ranges, np.arange(401), outdic['u650_orig'], outdic['v650_orig']*1000, density=1, linewidth=0.5, color='k')
    #ax.set_xticklabels(outticks)
    ...
    plt.contourf(ranges,np.arange(401), (outdic['pv'])/ outdic['cnte']*10000000, extend='both',  cmap='RdBu',levels=np.linspace(-0.5,0.5,10)) #
    plt.colorbar(label=r'10$^{-2}$ s$^{-1}$')
    contours = plt.contour(ranges, np.arange(401),(outdic['pv'] / outdic['cnte'])*1000, extend='both',levels=np.linspace(-3,3,11), cmap='RdBu')
    plt.clabel(contours, inline=True, fontsize=9, fmt='%1.2f')
    ax.set_yticklabels(np.array((np.linspace(0, 200 * 2, 9) - 200) * 3, dtype=int))
    #ax.set_xticklabels(outticks)
