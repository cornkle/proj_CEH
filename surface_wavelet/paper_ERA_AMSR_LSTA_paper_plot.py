...
def plot_amsr_paper_trio(h, eh):
    ...
    #name = "ERA5_cores_2hOverlap_AMSRE_SMALL_AMSL_new" #"ERA5_composite_cores_AMSRE_500w04_15k_minusMean"  # "ERA5_composite_cores_AMSRE_w1_15k_minusMean"
    #name = "ERA5_cores_DRY_SM0LT-3-1LT-1.5_noMeteosatFilter_AMSRE"
    name='ERA5_cores_2hOverlap_AMSRE_SMALL_AMSL_MCSfilter'
    ...
    u = (dic['u925']/ dic['cnte'])[4::st, 4::st]
    v = (dic['v925']/ dic['cnte'])[4::st, 4::st]
    levels = list(np.arange(-2.4,0,0.3)) + list(np.arange(0.3,2.7,0.3))
    ...
    plt.contour((dic['v925_orig'] / dic['cnte']), extend='both', colors='k', linewidths=5,
                           levels=[-50, 0.06, 50])

    contours = plt.contour((dic['t']-dic['tclim'])/ dic['cnte'], extend='both', #(dic['t']-dic['tclim'])
                           levels=[-1,-0.9,-0.8, -0.7, -0.6, -0.5, -0.4, -0.2, 0, 0.2, 0.4, 0.5, 0.6, 0.7, 0.8,0.9,1,1.2,1.4],
                           colors='k',linestyles = 'solid', linewidths = 1)
    plt.clabel(contours, inline=True, fontsize=9, fmt='%1.1f')
    #qu = ax1.quiver(xquiv, yquiv, u, v, scale=50)
    plt.axvline(x=200, linestyle='dashed', color='k', linewidth=1.2)
    ...
    plt.contourf(((dic['q']-dic['qclim'])*1000/ dic['cnte']), extend='both',  cmap='RdBu',levels=np.arange(-0.8,0.9,0.1)) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.colorbar(label='g kg-1')
    contours = plt.contour((dic['shear'] / dic['cnte']), extend='both',levels=np.arange(-15.25,-12.75,0.5), colors='k', linestyles='solid', linewidths=1) #np.arange(-15,-10,0.5)

    # contours = plt.contour((dic['u650'] / dic['cnte']), extend='both', levels=np.arange(-2, 2, 0.1), colors='k',
    #                        linestyles='solid', linewidths=1)  # np.arange(-15,-10,0.5)
 ...
 #   plt.contourf(((dic['lsta'])/ dic['cnt']), extend='both',  cmap='RdBu_r', vmin=-1.5, vmax=1.5) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    ...
    # plt.clabel(contours, inline=True, fontsize=9, fmt='%1.2f')
    qu = ax1.quiver(xquiv, yquiv, u, v, scale=15)
    qk = plt.quiverkey(qu, 0.9, 0.02,1, r'1 m s$^{-1}$',
                       labelpos='E', coordinates='figure')
