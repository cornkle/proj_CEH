...
def composite(h):
    ...
    file = '/users/global/cornkle/MCSfiles/blob_map_MCSs_-40-25000_JJAS_-50-points_dominant.nc'
    #h=17
    hour = h
    ...
    for r in res:
        ...
        missing_list.append(np.squeeze(r[4]))
        rres_list.append(np.squeeze(r[5]))
        rres2_list.append(np.squeeze(r[6]))
        rres3_list.append(np.squeeze(r[7]))
        rcnt_list.append(np.squeeze(r[8]))
        rmissing_list.append(np.squeeze(r[9]))
    ...
    missing_sum = np.nansum(np.squeeze(np.stack(cnt_list, axis=0)), axis=0)
    ...
    rmissing_sum = np.nansum(np.squeeze(np.stack(rcnt_list, axis=0)), axis=0)
    ...
    plt.title('Valid points cores, Nb cores: ' + str(np.max(cnt_sum)) + '| ' + str(hour).zfill(2) + '00UTC, Jun-Sep',
              fontsize=10)

    ax = f.add_subplot(222)

    plt.pcolormesh(rcnt_sum , cmap='viridis')
    plt.plot(extent, extent, 'bo')
    ax.set_xticklabels(np.array((np.linspace(0, extent*2, 5) - extent) * 3, dtype=int))
    ax.set_yticklabels(np.array((np.linspace(0, extent*2, 9) - extent) * 3, dtype=int))
    ax.set_xlabel('km')
    ax.set_ylabel('km')
    plt.colorbar(label='K')
    plt.title('Valid points random', fontsize=10)

    ax = f.add_subplot(223)

    plt.contourf((kernel2_sum / cnt_sum), cmap='RdBu_r',  vmin=24, vmax=28) #-(rkernel2_sum / rcnt_sum)
    plt.plot(extent, extent, 'bo')
    ax.set_xticklabels(np.array((np.linspace(0, extent*2, 5) - extent) * 3, dtype=int))
    ax.set_yticklabels(np.array((np.linspace(0, extent*2, 9) - extent) * 3, dtype=int))
    ax.set_xlabel('km')
    ...
    plt.contourf((rkernel2_sum / rcnt_sum), cmap='RdBu_r',  vmin=24, vmax=28) #-(rkernel2_sum / rcnt_sum)
    plt.plot(extent,extent, 'bo')
    ax.set_xticklabels(np.array((np.linspace(0, extent*2, 5) - extent) * 3, dtype=int))
    ax.set_yticklabels(np.array((np.linspace(0, extent*2, 9) - extent) * 3, dtype=int))
    ax.set_xlabel('km')
    ax.set_ylabel('km')
    plt.colorbar(label='K')
    plt.title('Random pattern full domain',
              fontsize=10)
    ...
    plt.savefig('/users/global/cornkle/figs/LSTA-bullshit/scales/new/composites_lsta/test/'+str(hour).zfill(2)+'newLST_allNan.png')#str(hour).zfill(2)+'00UTC_lsta_fulldomain_dominant<60.png')
    plt.close()
...
def cut_kernel(xpos, ypos, lsta_day2, month, lon, lat, t, parallax=None):
    ...
    if (np.sum(kernel!=0) < 0.5 * kernel.size) | (np.sum(np.isfinite(kernel)) < 0.5 * kernel.size):
        return
    ...
    missing = np.zeros_like(kernel)
    cnt[np.isfinite(kernel)] = 1
    missing[np.isnan(kernel)]= 1

    return ano, kernel, kernel3, cnt, missing
...
def file_loop(fi):
    ...
    if (fi['time.hour'].values) <= 15:
        ...
        daybefore = date #+ dayd
    try:
        lsta = xr.open_dataset('/users/global/cornkle/data/OBS/MSG_LSTA/lst_new/' \
                           'lst_daily_' + str(daybefore.year) + str(daybefore.month).zfill(2) + str(daybefore.day).zfill(2) + '.nc')
    except OSError:
        return
    ...
    #lsta_day2.values[np.isnan(lsta_day2.values)] = -0.2
    ...
    xfi = fi.shape[1]
    yfi = fi.shape[0]
    randx = np.random.randint(0,xfi,200)
    randy = np.random.randint(0,yfi,200)
    posr = (randy, randx)
    ...
    for y, x in zip(posr[0], posr[1]):
        ...
        ypos = int(ypos[0])
        try:
            ano, kernel2, kernel3, cnt, missing = cut_kernel(xpos, ypos, lsta_day2, daybefore.month, plon, plat, -40, parallax=False)
        except TypeError:
            continue
    ...
    if len(kernel_list) == 1:
      return None
        ...
        cnt_sum = np.nansum(np.squeeze(np.stack(cnt_list, axis=0)), axis=0)
        missing_sum = np.nansum(np.squeeze(np.stack(missing_list, axis=0)), axis=0)
        ...
        rcnt_sum = np.nansum(np.squeeze(np.stack(rcnt_list, axis=0)), axis=0)
        rmissing_sum = np.nansum(np.squeeze(np.stack(rmissing_list, axis=0)), axis=0)

    print('Returning')

    return (kernel_sum, kernel2_sum, kernel3_sum, cnt_sum, missing_sum, rkernel_sum, rkernel2_sum, rkernel3_sum, rcnt_sum, rmissing_sum )
