...
def composite(h, eh):
    ...
    for year in np.arange(2006, 2011):
        ...
        pkl.dump(dic, open(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/ERA5_composite_cores_AMSRE_500w04_15k_minusMean"+str(eh) + "UTCERA"+str(hour).zfill(2)+'_'+str(year)+"_small_cores.p", "wb"))
        del dic
        print('Dumped file')
...
def plot_AMSR_all(h, eh):
    ...
    plt.contourf(((dic['lsta-3'] + dic['lsta-2']) / (dic['cnt-3'] + dic['cnt-2'])), cmap='RdBu', levels=np.linspace(-1,1,10), extend='both') #-(rkernel2_sum / rcnt_sum)
...
#
    ...
    plt.contourf(((dic['lsta-1'] + dic['lsta0']) / (dic['cnt-1'] + dic['cnt0'])), extend='both',  cmap='RdBu',levels=np.linspace(-1,1,10)) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
 ...
 #   plt.contourf(((dic['lsta'])/ dic['cnt']), extend='both',  cmap='RdBu_r', vmin=-1.5, vmax=1.5) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    ...
    #   plt.contourf(((dic['lsta'])/ dic['cnt']), extend='both',  cmap='RdBu_r', vmin=-1.5, vmax=1.5) # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.contourf(((dic['v925_orig']) / dic['cnte']) , extend='both', cmap='RdBu', levels=np.linspace(-1,1,10))  # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.colorbar(label='K')
    plt.plot(extent, extent, 'bo')
    contours = plt.contour((dic['tciwlow'] / dic['cnte']), extend='both', cmap='viridis') #np.arange(-15,-10,0.5)
    plt.clabel(contours, inline=True, fontsize=9, fmt='%1.2f')
