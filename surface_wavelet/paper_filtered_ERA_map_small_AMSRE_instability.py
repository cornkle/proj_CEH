...
def composite(h, eh):
    ...
    for year in np.arange(2006, 2011):
        ...
        pkl.dump(dic, open(cnst.network_data + "figs/LSTA/corrected_LSTA/new/ERA5/core_txt/ERA5_cores_"+key+"_AMSRE_DRY20_instability_HOURLY_BUDGET"+str(eh) + "UTCERA"+str(hour).zfill(2)+'_'+str(year)+".p", "wb"))
        del dic
        print('Dumped file')
...
def cut_kernel(xpos, ypos, arrlist, dist, era=False, msg=False, cmorph=False):
    ...
    cntcin = np.zeros_like(kernel)
    cntcinclim = np.zeros_like(kernel)
    cntera[np.isfinite(vdic['t'])] = 1
    cntcin[np.isfinite(vdic['cin'])] = 1
    cntcinclim[np.isfinite(vdic['cin_clim'])] = 1
    ...
    return smoutlist,  scntlist, vdic,cntera, probm, cntmsg, probcm, cntcm, cntcin, cntcinclim
...
def get_previous_hours(storm_date, lsta_date, ehour, refhour):
    ...
    file = '/media/ck/Elements/Africa/WestAfrica/ERA5/hourly/LSTA_instability/'#cnst.ERA5

    try:
        cmp = xr.open_dataset(file + 'ERA5_'+str(edate.year)+'_' + str(edate.month).zfill(2) + '_pl.nc')
        cmp = u_darrays.flip_lat(cmp)
    except:
        print('ERA5 pl not found')
        return None

    try:
        css = xr.open_dataset(cnst.ERA5 + 'hourly/pressure_levels/ERA5_'+str(t1_MCScheck.year)+'_' + str(t1_MCScheck.month).zfill(2) + '_pl.nc')
        css = u_darrays.flip_lat(css)
    except:
        print('ERA5 srfc not found')
        return None

    csm = xr.open_dataset(file + 'ERA5_' + str(edate.year) + '_' + str(edate.month).zfill(2) + '_srfc.nc')
        #file + 'hourly/surface/ERA5_' + str(edate.year) + '_' + str(edate.month).zfill(2) + '_srfc.nc')
    csm = u_darrays.flip_lat(csm)


    pl_clim = xr.open_dataset(cnst.ERA5 + 'monthly/synop_selfmade/CLIM_2006-2010/ERA5_2006-2010_CLIM_'+str(edate.month).zfill(2)+'-'+str(edate.day).zfill(2)+'-'+str(edate.hour).zfill(2)+'_pl_instability.nc').load()
    ...
    esrfc_clim = xr.open_dataset(
        cnst.ERA5 + 'monthly/synop_selfmade/CLIM_2006-2010/ERA5_2006-2010_CLIM_'+str(edate.month).zfill(2)+'-'+str(edate.day).zfill(2)+'-'+str(edate.hour).zfill(2)+'_srfc_instability.nc').load()
    esrfc_clim = u_darrays.flip_lat(esrfc_clim)
    esrfc_clim = esrfc_clim.rename({'lat': 'latitude', 'lon': 'longitude'})
    ...
    csm = csm.sel(longitude=slice(-12.5, 12.5), latitude=slice(7, 23))
    srfc_clim = esrfc_clim.sel(longitude=slice(-12.5, 12.5), latitude=slice(7, 23))
    ...
    cape = scm['cape'].squeeze()
    cape_clim = srfc_clim['cape'].squeeze()
    cin = scm['cin']
    cin_clim = srfc_clim['cin'].squeeze()

    t = cmm['t'].squeeze()
    ttclim = pl_clim['t'].squeeze() #* 1000

    div = cmm['d'].squeeze()
    level_low = 925
    thetae_925 = u_met.theta_e(level_low, cmm['t'].squeeze().values - 273.15,
                            cmm['q'].squeeze())

    theta_925 = u_met.theta(level_low, cmm['t'].squeeze() - 273.15)


    thetae_clim_925 = u_met.theta_e(level_low, pl_clim['t'].squeeze().values - 273.15,
                                 pl_clim['q'].squeeze())

    theta_clim_925 = u_met.theta(level_low, pl_clim['t'].squeeze() - 273.15)

    theta_e = thetae_925
    theta_e_clim = thetae_clim_925

    q = cmm['q'].squeeze()
    qclim = pl_clim['q'].squeeze()


    cm['cin'] = cin
    cm['cape'] = cape
    cm['cin_clim'] = cin_clim
    cm['cape_clim'] = cape_clim
   # ipdb.set_trace()
...
def file_loop(df):
    ...
    cnte_cin_sum = np.zeros((dist * 2 + 1, dist * 2 + 1))
    cnte_ccin_sum = np.zeros((dist * 2 + 1, dist * 2 + 1))
    ...
    for dids, dit in df.iterrows():
        ...
        try:
            smoutlist, scntlist, vdic, cntera, probm, cntmsg, probcm, cntcm, cntcin, cntccin = cut_kernel(xpos, ypos, smlist, dist, era=probs_on_lsta, msg=probsm_on_lsta, cmorph=probscm_on_lsta)
        except TypeError:
            print('TypeError')
            continue
        ...
        cnte_cin_sum = np.nansum(np.stack([cnte_cin_sum, cntcin]), axis=0)
        cnte_ccin_sum = np.nansum(np.stack([cnte_ccin_sum, cntccin]), axis=0)
    ...
    outlist = smkernels + smcnt + [cnte_sum, cntm_sum, probm_sum, cntc_sum, probc_sum, cnte_cin_sum, cnte_ccin_sum]
    outnames = ['lsta0', 'lsta-1', 'lsta-2', 'lsta-3', 'cnt0', 'cnt-1', 'cnt-2', 'cnt-3', 'cnte', 'cntm', 'probmsg',
                'cntc', 'probc', 'cnt_cin', 'cnt_ccin']
    for ek in edic.keys():
        outnames.append(ek)
