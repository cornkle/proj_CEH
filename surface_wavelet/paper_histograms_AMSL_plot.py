...
def plot(hour, key):
    ...
    cinput = np.array(data)[1]
    rinput = np.array(random)[1]

    cinput = cinput[np.isfinite(cinput)]
    rinput = rinput[np.isfinite(rinput)]

    print('NUMBEROFCORE', cores)
...
def plot_double(hour):
    ...
    for id,h in enumerate(hour):
        path = cnst.network_data + 'figs/LSTA/corrected_LSTA/new/wavelet_coefficients/'
        dic = pkl.load(open(path+"AMSR_histograms_"+str(h).zfill(2)+"_SMFINITE.p", "rb"))
        ...
        cinput = np.array(data)[id]
        rinput = np.array(random)[id]
    ...
    plt.tight_layout()
    plt.savefig(path + '/2hOverlap/AMSRE_histo_perHOUR_'+str(hour[0])+'.png')
