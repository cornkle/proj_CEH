...
def composite(h, eh):
    ...
    dic = u_parallelise.era_run_arrays(1,file_loop,msg) #'rano', 'rregional', 'rcnt',
...
def plot_gewex(h, ehour):
    ...
    plt.contourf(((dic['w400'])*100/ dic['cntp']), extend='both',  cmap='RdBu') # #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.colorbar(label='m s-1')

    contours = plt.contour((dic['v925raw'] *100/ dic['cntp']), extend='both', cmap='viridis', levels=np.linspace(-10,10,10)) #np.arange(-15,-10,0.5)
    plt.clabel(contours, inline=True, fontsize=9, fmt='%1.2f')
    ...
    ax1 = f.add_subplot(236)
    plt.contourf(((dic['rh'])/ dic['cntp']), extend='both',  cmap='RdBu', levels=np.linspace(-10,10,10)) #  tciw  #, levels=np.arange(1,5, 0.5), levels=np.arange(10,70,5)
    plt.colorbar(label='m s-1')
