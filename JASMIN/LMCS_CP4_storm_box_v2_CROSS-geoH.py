...
def file_save(cp_dir, out_dir, vars, datestring, box, tthresh):
    ...
    for idx, outv in enumerate(keys):
        ...
        if ATAG == 'anom' :
            ...
            clim_files = '/home/users/cornkle/lmcs/cklein/CP_models/MCS_files/climatology/'+ctag
        ...
        try:
            da = xr.DataArray(dar.values,
                          coords={'time': dar.time, 'pressure': dar.pressure, 'latitude': dar.latitude.values,
                                  'longitude': dar.longitude.values, },
                           dims=['pressure','latitude', 'longitude'])
        except:
                        da = xr.DataArray(dar.values,
                              coords={'time': dar.time, 'latitude': dar.latitude.values,
                                      'longitude': dar.longitude.values, },
    ...
    for gi in goodinds:
        ...
        if (np.sum(np.isnan(filtx['geoH_cross']))/filtx['geoH_cross'].size) > 0.1:
            continue
        ...
        savefile_x = out_dir + os.sep + date.strftime('%Y-%m-%d_%H:%M:%S') + '_XDIR_' + str(gi) + '_lonXlat_'+str(np.round(plon,1))+'_'+str(np.round(plat,1))+'_geoH.nc'
        savefile_y = out_dir + os.sep + date.strftime('%Y-%m-%d_%H:%M:%S') + '_YDIR_' + str(gi) + '_lonXlat_'+str(np.round(plon,1))+'_'+str(np.round(plat,1))+'_geoH.nc'
...
vars['lw_out_PBLtop'] = ([], h, (0,0,0), 'lw_out_PBLtop', '')  ### Input in BRIGHTNESS TEMPERATURES!! (degC) # add srfc again 
vars['lsRain'] =  ([], h, (0,0,0), 'lsRain', '')
vars['geoH_cross'] = (plevs, 12, (0,0,0), 'geoH_pl', '')
...
for d in datelist:
    ...
    testfiles = glob.glob(out_path + os.sep + d[0:4] + '-' + d[4:6] + '-' + d[6:8] + '_' + str(h) + '*DIR_*_geoH.nc')
