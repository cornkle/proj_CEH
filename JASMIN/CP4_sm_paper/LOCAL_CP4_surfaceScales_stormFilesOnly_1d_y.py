...
for idx, dats in enumerate([hist, fut]):
    ...
    for sf in dats:  ########restricted files for testing  [0:5000]
        ...
        midpoint = 58
        dy = 11
        lines = sda[var].isel(longitude=slice(midpoint-dy, midpoint+dy)).mean('longitude')
        ...
        coeffs, power, scales, period = wObj.applyWavelet_1d(normed='scale', direction='y')#, le_thresh=0, fill=0)

        coeffline = np.nanmean(coeffs[:,:,midpoint-dy:midpoint+dy], axis=2)
        powerline = np.nanmean(power[:,:,midpoint-dy:midpoint+dy], axis=2)
        #ipdb.set_trace()
...
pkl.dump(strct_values, open(cnst.network_data+ 'data/LMCS/CP4_study_saves/'+var+'_xcross_-11to11_'+hour+'_1d_y.p','wb'))
pkl.dump(strct_scale, open(cnst.network_data+ 'data/LMCS/CP4_study_saves/'+var+'_wcoeffs_-11to11_'+hour+'_1d_y.p','wb'))
pkl.dump(strct_power, open(cnst.network_data+ 'data/LMCS/CP4_study_saves/'+var+'_power_-11to11_'+hour+'_1d_y.p','wb'))