...
def add_derived_fields(ds, dx_m=4400, p_low_hpa=925, p_mid_hpa=650):
    """Add divergence and simple two-level thermodynamic proxies."""

    # Assumptions:
    # t_srfc and t_mid: degC
    # q_srfc and q_mid: g kg-1
    # u_srfc and v_srfc: m s-1
    t_low = ds["t_srfc"].values * units.degC
    t_mid = ds["t_mid"].values * units.degC
    q_low = ds["q_srfc"].values * units("g/kg")
    q_mid = ds["q_mid"].values * units("g/kg")
    ...
    # MetPy defaults to x_dim=-1 and y_dim=-2, i.e. arrays ordered (..., y, x).
...
def run_case(timetag, anomtag, processes=4):
    ...
    with xr.open_dataset(files[0]) as dummy:
        dummy = add_derived_fields(dummy)
