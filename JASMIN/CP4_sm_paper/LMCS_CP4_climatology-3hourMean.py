...
def file_save(cp_dir, out_dir, vars, datestring, box):
    ...
    for idx, outv in enumerate(vars):
        ...
        savefile = out_dir + os.sep +  outv+'_'+datestring[4:6] + '-' + datestring[6:8] + '_'+ str(HOUR) + '_'+FTAG+'_3HourMean.nc'
        ...
        for yy in range(1998,2007):
                ...
                for dd in [7,6,5,4,3,2,1,0]:
                    ...
                    try:
                        ...
                        continue
                    mmeanarr = mmeanarr[(mmeanarr['time.hour'] <= HOUR) & (mmeanarr['time.hour'] >= HOUR-2)].mean('time').squeeze()
