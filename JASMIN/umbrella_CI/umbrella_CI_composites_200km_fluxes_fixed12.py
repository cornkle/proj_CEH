...
PRESSURE_LEVELS = {"u600": 600.0, "v600": 600.0, "w925": 925.0, "w850": 850.0, "w600": 600.0}
VARIABLES = {
    ...
    "w925": ("apverc.pp", "m01s15i242"),
    "w850": ("apverc.pp", "m01s15i242"),
    "w600": ("apverc.pp", "m01s15i242"),
    "t1p5": ("apvera.pp", "m01s03i236"),
    "sh_flux": ("apvera.pp", "m01s03i217"),
    "lh_flux": ("apvera.pp", "m01s03i234"),
}
CUBE_SELECTOR = {"rain": "time_mean", "sh_flux": "prefer_time_mean", "lh_flux": "prefer_time_mean"}
TIME_METHOD = {
    "q_low": "exact", "u10": "exact", "v10": "exact", "sm": "exact",
    "rain": "exact", "olr": "nearest", "u600": "nearest", "v600": "nearest",
    "w925": "nearest", "w850": "nearest", "w600": "nearest",
    "t1p5": "exact", "sh_flux": "exact", "lh_flux": "exact",
}
COMPOSITE_FIELDS = [
    "sm_anomaly", "q_low", "t1p5", "ctt", "rain", "sh_flux", "lh_flux",
    "shear", "shear_u", "shear_v", "div10", "w925", "w850", "w600",
    "u10", "v10", "u600", "v600",
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("/gws/ssde/j25b/kscale/USERS/cklein/umbrella/full_composites_200km_fluxes_fixed12"))
    parser.add_argument("--year", type=int, default=2020)
    parser.add_argument("--ci-hours", type=int, nargs="+", default=[13, 14, 15])
    parser.add_argument("--lag-hours", type=int, nargs="+", default=[-3, -2, -1],
...
class CompositeProcessor:
    ...
    def select_vertical_level(self, cube, variable: str):
        ...
        if variable in PRESSURE_LEVELS:
            target_hpa = PRESSURE_LEVELS[variable]
            pressure = cube.coord("pressure").copy()
            pressure.convert_units("hPa")
            index = int(np.argmin(np.abs(pressure.points - target_hpa)))
            if not np.isclose(pressure.points[index], target_hpa):
                raise ValueError(f"{target_hpa} hPa is unavailable; available={pressure.points}")
    ...
    def load_base_cube(self, stream: str, stash: str, cycle_string: str, selector: str = "single"):
        ...
        constraint = iris.Constraint(cube_func=lambda cube: str(cube.attributes.get("STASH", "")) == stash)
        cubes = list(iris.load(str(path), constraint))
        all_cubes = cubes
        if selector in ("time_mean", "prefer_time_mean"):
            mean_cubes = [cube for cube in all_cubes if self.has_time_mean(cube)]
            if mean_cubes:
                cubes = mean_cubes
            elif selector == "prefer_time_mean" and len(all_cubes) == 1:
                cubes = all_cubes
                logging.warning("%s has no time-mean cell method; using the sole available cube", stash)
            else:
                cubes = []
        if len(cubes) != 1:
            details = [(cube.name(), cube.shape, str(cube.units), cube.cell_methods) for cube in all_cubes]
            raise ValueError(f"Expected one cube for {stash}, selector={selector}; selected {len(cubes)}; available={details}")
        return cubes[0]
    ...
    def effective_times(self, cube, variable: str) -> list[pd.Timestamp]:
        ...
        use_upper_bound = variable == "rain" or (variable in ("sh_flux", "lh_flux") and self.has_time_mean(cube))
        values = time.bounds[:, 1] if use_upper_bound and time.has_bounds() else time.points
    ...
    def process_one_case(self, case: pd.Series, fields: dict):
        ...
        for variable, cube in fields.items():
            data, source_x, source_y = self.extract_local_data(cube, lon0, lat0, source_half_width)
        ...
        rotated_scalar = {
            ...
            for name in ("sm", "q_low", "t1p5", "rain", "sh_flux", "lh_flux", "w925", "w850", "w600")
        }
        with np.errstate(invalid="ignore", divide="ignore"):
            local_ctt = np.where(np.isfinite(local["olr"]) & (local["olr"] > 0.0),
                                 np.power(local["olr"] / STEFAN_BOLTZMANN, 0.25) - 273.15, np.nan)
        rotated_scalar["ctt"] = self.interpolate_rotated(local_ctt, *coordinates["olr"], rotation_angle)
        earth_wind = {
            name: self.interpolate_rotated(local[name], *coordinates[name], rotation_angle)
        ...
        return {
            ...
            "t1p5": rotated_scalar["t1p5"] - 273.15,
            "ctt": rotated_scalar["ctt"],
            "rain": rotated_scalar["rain"] * 3600,
            "sh_flux": rotated_scalar["sh_flux"],
            "lh_flux": rotated_scalar["lh_flux"],
            "shear": np.hypot(shear_u, shear_v),
            "shear_u": shear_u,
            "shear_v": shear_v,
            "div10": divergence,
            "w925": rotated_scalar["w925"], "w850": rotated_scalar["w850"], "w600": rotated_scalar["w600"],
            "u10": u10, "v10": v10, "u600": u600, "v600": v600,
        }

    def load_fields_for_time(self, target_time: pd.Timestamp) -> dict:
        fields = {}
        used = {}
        ...
        if not self.args.lazy_subsets:
            logging.info("Preloading eight global 2-D fields for %s", target_time)
            for cube in fields.values():
                _ = cube.data
        return fields
...
def write_netcdf(path: Path, processor: CompositeProcessor, means, counts, successful, args):
    ...
    units = {"sm_anomaly": "kg m-2", "q_low": "g kg-1", "t1p5": "degree_Celsius", "ctt": "degree_Celsius",
             "rain": "mm h-1", "sh_flux": "W m-2", "lh_flux": "W m-2",
             "shear": "m s-1", "shear_u": "m s-1", "shear_v": "m s-1", "div10": "s-1",
             "w925": "m s-1", "w850": "m s-1", "w600": "m s-1",
             "u10": "m s-1", "v10": "m s-1", "u600": "m s-1", "v600": "m s-1"}
    for field, unit in units.items():
        ds[field].attrs["units"] = unit
    ds["ctt"].attrs.update({"long_name": "OLR-derived effective cloud-top temperature", "source_stash": "m01s02i205"})
    ds["t1p5"].attrs.update({"long_name": "1.5 m air temperature", "source_stash": "m01s03i236"})
    ds["sh_flux"].attrs.update({"long_name": "surface upward sensible heat flux", "source_stash": "m01s03i217", "positive": "upward"})
    ds["lh_flux"].attrs.update({"long_name": "surface upward latent heat flux", "source_stash": "m01s03i234", "positive": "upward"})
    for level in (925, 850, 600):
        ds[f"w{level}"].attrs.update({"long_name": f"upward air velocity at {level} hPa", "source_stash": "m01s15i242", "positive": "upward"})
...
def run_one_lag(args: argparse.Namespace, cases_with_filters: pd.DataFrame) -> None:
    ...
    for group_index in range(start_group, len(time_groups)):
        ...
        try:
            fields = processor.load_fields_for_time(target_time)
        ...
        fields = None
        processor.load_base_cube.cache_clear()
        gc.collect()
    ...
    nc_path = run_dir / "200km_fluxes.nc"
    ...
    if not args.no_plots:
        plots = [
            ...
            ("ctt", "OLR-derived cloud-top temperature", "YlGnBu", r"$^\circ$C", False, 1, False),
            ("t1p5", "1.5 m air temperature", "RdYlBu_r", r"$^\circ$C", False, 1, False),
            ("sh_flux", "Surface upward sensible heat flux", "RdBu_r", "W m$^{-2}$", True, 1, False),
            ("lh_flux", "Surface upward latent heat flux", "BrBG", "W m$^{-2}$", True, 1, False),
            ("w925", "Upward air velocity at 925 hPa", "RdBu_r", "m s$^{-1}$", True, 1, False),
            ("w850", "Upward air velocity at 850 hPa", "RdBu_r", "m s$^{-1}$", True, 1, False),
            ("w600", "Upward air velocity at 600 hPa", "RdBu_r", "m s$^{-1}$", True, 1, False),
...
def main() -> None:
    ...
    # Alternative design: different CI-hour samples, all evaluated at 12 UTC.
    # 13 UTC CIs -> t=-1, 14 UTC CIs -> t=-2, 15 UTC CIs -> t=-3.
    required_hours = [13, 14, 15]
    cases = cases[cases["hour"].isin(required_hours)].copy()
    if cases.empty:
        raise RuntimeError(f"No CIs found for hours {required_hours}")
    ...
    fixed_12_mapping = {-1: 13, -2: 14, -3: 15}
    requested_lags = [int(lag) for lag in args.lag_hours]
    invalid_lags = [lag for lag in requested_lags if lag not in fixed_12_mapping]
    
    if invalid_lags:
        raise ValueError(f"The fixed-12UTC experiment only supports lags -1, -2 and -3; got {invalid_lags}")
    
    for lag in requested_lags:
        ci_hour = fixed_12_mapping[lag]
        lag_args = argparse.Namespace(**vars(args))
        lag_args.ci_hours = [ci_hour]
        lag_args.lag_hours = lag
        hour_cases = cases[cases["hour"] == ci_hour].copy()
    
        if hour_cases.empty:
            logging.warning("No CI cases found at %02d UTC; skipping t=%+d h", ci_hour, lag)
            continue
    
        logging.info("Starting fixed-12UTC composite: %02d UTC CIs at t=%+d h", ci_hour, lag)
        run_one_lag(lag_args, hour_cases)
